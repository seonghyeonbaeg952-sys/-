...
export function TextRunContent({ text, runs, offset = 0, lineBreaks = false }: {
  ...
  for (const run of runs) {
    ...
    const style = { fontFamily: font, fontSize: size, '--site-copy-font': font, '--site-copy-size': size } as CSSProperties
    ...
    children.push(createElement('smyc-copy', { key: `style-${start}`, className: 'site-copy-format', 'data-site-copy-font': font ? '' : undefined, 'data-site-copy-size': size ? '' : undefined, style }, lines(text.slice(start, end), lineBreaks)))
    cursor = end
  }
...
export function FormattedCopy({ page, id, text, fullText = text, offset = 0, children, lineBreaks }: Props) {
  ...
  if (!runs.some(run => run.start < offset + text.length && run.end > offset) || fullText.slice(offset, offset + text.length) !== text) return children ?? text
  ...
  return createElement('smyc-text', { className: 'site-copy-text' }, <TextRunContent text={text} runs={runs} offset={offset} lineBreaks={lineBreaks} />)
}
