  ...
  return (
    <div className="contact-atelier">
      ...
      <header className="contact-atelier__intro contact-atelier__shell">
        {activeSection !== 'all' ? <Link className="contact-atelier__back" to="/contact">{t('back')}</Link> : null}
        <p className="contact-atelier__eyebrow">{copyText("contact", "contact.fixed.ContactPage.f34c03131f", "SEOUL MOTET YOUTH CHOIR")}</p>
        <div className="contact-atelier__title-row">
          <h1>{title}</h1>
          <p>{activeSection === 'support' ? (supportSettings?.subtitle || '후원 방식과 약정 내용을 확인해 주세요.') : <CopyLines text={t('description')} />}</p>
        </div>
      </header>
      ...
      {contactData.error ? <section className="contact-atelier__state contact-atelier__shell" role="alert"><h2>{copyText("contact", "contact.fixed.ContactPage.f51f71b9d6", "문의 정보를 불러오지 못했습니다.")}</h2><p>{copyText("contact", "contact.fixed.ContactPage.4ac734f478", "연결 상태를 확인하고 다시 시도해 주세요. 운영 정보가 확인되기 전에는 약정서를 표시하지 않습니다.")}</p><button type="button" className="contact-atelier__action" onClick={contactData.refetch}>{copyText("contact", "contact.fixed.ContactPage.196856a662", "다시 시도")}</button></section> : null}
        ...
        <section className="contact-atelier__section contact-atelier__support" id="support" aria-labelledby="contact-support-title">
          <div className="contact-atelier__shell contact-atelier__columns">
            <div className="contact-atelier__aside">
              <p className="contact-atelier__eyebrow">{t('support')}</p>
              <h2 id="contact-support-title"><CopyLines text={t('supportTitle')} /></h2>
              <p>{supportSettings?.description || siteSettings.support_text || '후원 관련 안내는 문의를 통해 도와드리겠습니다.'}</p>
              <Link className="contact-atelier__action contact-atelier__action--primary" to={supportSettings ? '/contact?section=support#support' : '/contact?section=inquiry&type=support#form'}>{supportSettings ? t('pledgeAction') : t('supportInquiry')} <span aria-hidden="true">→</span></Link>
            </div>
            <div>
              <h3>{t('regular')}</h3>
              {supportSettings ? <>
                <dl className="contact-atelier__amounts">
                  {supportSettings.individual_amounts.length ? <div><dt>{t('individual')}</dt><dd>{copyText("contact", "contact.fixed.ContactPage.450d32b52e", "월 ")}{formatSupportAmounts(supportSettings.individual_amounts)}</dd></div> : null}
                  {supportSettings.corporate_amounts.length ? <div><dt>{t('corporate')}</dt><dd>{copyText("contact", "contact.fixed.ContactPage.450d32b52e", "월 ")}{formatSupportAmounts(supportSettings.corporate_amounts)}</dd></div> : null}
                </dl>
                {supportSettings.allow_custom_amount ? <p className="contact-atelier__support-note">{copyText("contact", "contact.fixed.ContactPage.c43d9df174", "다른 금액도 직접 입력하실 수 있습니다.")}</p> : null}
                <p className="contact-atelier__support-note">{copyText("contact", "contact.fixed.ContactPage.bb4b9f91d5", "약정서를 보내도 결제되거나 자동 출금되지 않습니다.")}</p>
                <p>{copyText("contact", "contact.fixed.ContactPage.a18802069e", "후원 관련 자세한 안내는 문의를 통해 도와드리겠습니다.")}</p>
              </> : <p className="contact-atelier__support-note">{copyText("contact", "contact.fixed.ContactPage.0fe35f361f", "현재 후원 약정 접수를 준비하고 있습니다. 후원 방식과 금액은 문의를 통해 확인해 주세요.")}</p>}
            </div>
          </div>
        </section>
      ) : null}
      ...
      {ready && activeSection === 'support' ? (
        supportSettings ? <div className="contact-atelier__shell"><SupportPledgeForm settings={supportSettings} siteSettings={siteSettings} /><p className="contact-atelier__support-note"><Link className="contact-atelier__action" to="/contact?section=inquiry&type=support#form">{copyText("contact", "contact.fixed.ContactPage.cb4a92fb56", "후원에 대해 문의하기 ")}<span aria-hidden="true">→</span></Link></p></div>
          : <section className="contact-atelier__state contact-atelier__shell"><h2>{copyText("contact", "contact.fixed.ContactPage.5a0e526131", "후원 약정 접수를 준비하고 있습니다.")}</h2><p>{copyText("contact", "contact.fixed.ContactPage.a043c327a1", "운영 설정이 확인되기 전에는 온라인 약정서를 표시하지 않습니다. 아래 문의 양식으로 연락해 주세요.")}</p></section>
      ) : null}
      ...
      {ready && (showAll || activeSection === 'sponsors') ? (
        <section id="contact-sponsors" className="contact-atelier__sponsors contact-atelier__shell">
          {sponsors.length > 0 ? <SponsorsSection sponsors={sponsors} /> : <div className="contact-atelier__sponsor-empty" id="sponsors"><h2>{t('sponsorTitle')}</h2><p><CopyLines text={t('sponsorEmpty')} /></p></div>}
        </section>
      ) : null}
        ...
        <section className="contact-atelier__section contact-atelier__location" id="location" aria-labelledby="contact-location-title">
          <div className="contact-atelier__shell">
            <div className="contact-atelier__columns">
              <div>
                <p className="contact-atelier__eyebrow">{t('location')}</p>
              ...
              <dl className="contact-atelier__contacts">
                {siteSettings.phone?.trim() ? <div><dt>{t('phone')}</dt><dd><a href={'tel:' + siteSettings.phone.replace(/[^0-9+]/g, '')}>{siteSettings.phone}</a></dd></div> : null}
                {siteSettings.email?.trim() ? <div><dt>{t('email')}</dt><dd>{siteSettings.email}</dd></div> : null}
                {siteSettings.fax?.trim() ? <div><dt>{copyText("contact", "contact.fixed.ContactPage.6f110978f5", "FAX")}</dt><dd>{siteSettings.fax}</dd></div> : null}
              </dl>
            </div>
            {maps.embedSrc || location?.image_url ? <details className="contact-atelier__location-media"><summary>{t('map')}</summary>
