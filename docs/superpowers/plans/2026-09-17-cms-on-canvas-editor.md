...
> **For agentic workers:** 설치된 `executing-plans` 스킬을 사용해 아래 작업을 검증 단위별로 수행한다. 이 문서는 2026-09-17 설계에서 시작했으며 사용자 적용 지시 후 2026-09-18 실제 구현을 진행했다. 최신 구현·20회 검증 증거는 `docs/cms-office-editor-verification-2026-09-18.md`, 플로팅 Figma 시안은 `docs/cms-floating-figma-review-2026-09-18.md`를 참조한다. 아래 미확인 체크박스를 전체 완료로 바꾸지 않는다.
...
- 최초 설계 단계의 산출물은 계획 문서와 Figma 시안이었다. 2026-09-18 사용자 적용 지시 이후에만 CMS 편집 코드와 허용 서식 validator를 변경했다. 공개 기본 디자인과 영어·스크롤바 기능은 변경하지 않는다.
...
2026-09-18 추가 조사: [Microsoft Word·PowerPoint 공식 자료 8개](../../research/2026-09-18-office-editor-microsoft.md), [한컴 한/글 공식 자료 11개](../../research/2026-09-18-office-editor-hancom.md)는 각각 독립 원장으로 기록하며 위 10건과 합산하지 않는다. 이 비교를 반영해 아래 계약을 갱신했다. Esc는 입력을 버리는 취소가 아니라 안전하게 로컬 초안에 반영한 뒤 요소 선택으로 돌아가는 동작이고, 명시적인 `이번 편집 되돌리기`만 현재 입력을 취소한다. `F2`와 `편집 마침`도 같은 반영 경로를 사용한다. `Ctrl+Enter`는 Word·한/글의 쪽 나누기와 혼동되므로 CMS 마침 단축키나 별칭으로 사용하지 않는다. 모든 제품·플랫폼이 같은 키를 쓴다는 주장은 하지 않는다.
...
7. `F2`, `편집 마침`, 또는 조합 중이 아닌 `Esc`로 현재 활성 입력 트랜잭션을 부모의 로컬 초안에 안전하게 반영한 뒤 블록 선택 상태로 돌아간다. 부모 검증·승인이 실패하거나 응답이 없으면 버퍼와 선택을 보존하고 완료로 표시하지 않는다. 현재 입력을 포기하려면 명시적인 `이번 편집 되돌리기` 버튼을 사용한다. 이 버튼은 활성 입력만 취소하며 이전에 확정한 다른 변경에는 영향을 주지 않는다. 이 동작들은 모두 서버 초안 저장·공개와 별개다.
8. 미리보기로 전환하면 윤곽선과 편집 입력을 제거한다. 접수·업로드 등 실제 전송은 기존 미리보기 차단 정책을 유지한다.

도구 모음 클릭은 선택 범위를 유지한 채 현재 입력 버퍼의 서식을 바꾼다. 다른 블록이나 빈 캔버스를 클릭해도 현재 입력을 조용히 버리거나 확정하지 않는다. 선택 이동 의도를 보관하고 `편집 마침` 또는 `이번 편집 되돌리기`를 인라인으로 안내한 뒤 이동한다. 미리보기·페이지·기기 전환도 미완료 입력이 있으면 같은 원칙을 따른다.
...
- 화면에 보이는 `편집 마침`은 부모 로컬 초안에 안전하게 반영하고 `이번 편집 되돌리기`는 현재 입력 트랜잭션만 취소한다. 바깥 탭이나 키보드 닫힘만으로 입력을 버리지 않는다. 반영 실패 시 입력 화면과 버퍼를 유지한다.
...
- 블록 선택 상태의 Enter는 입력 진입, 입력 상태의 Enter는 허용된 줄바꿈이다. 조합 중이 아닌 `F2`·`Esc`는 `편집 마침`과 같은 부모 로컬 초안 반영 동작이다. 승인이 확인된 뒤 블록 선택으로 돌아가며 실패하면 버퍼를 유지한다. 블록 선택 상태의 Esc는 선택 해제이며 이전 초안 변경을 취소하지 않는다. `Ctrl+Enter`는 마침에 연결하지 않는다. 현재 입력 취소는 Tab으로 접근할 수 있는 `이번 편집 되돌리기` 버튼으로 제공한다.
- 선택 범위가 없을 때 Delete가 문단 전체를 지우지 않는다. 블록 삭제 기능은 이번 범위에 없다.
- 글자 선택은 브라우저 기본 키보드 조작을 유지하고, 서식 도구 모음은 Tab으로 접근한다. 화면에는 포커스 표시가 남는다.
- 실행 취소·다시 실행 버튼에 이름을 제공하고 활성 편집기 안에서만 Ctrl/Cmd+Z, Ctrl/Cmd+Shift+Z를 연결한다. Windows Ctrl+Y 별칭은 지원 여부를 명시하고 전역으로 가로채지 않는다. 한/글에서는 Ctrl+Y가 한 줄 삭제이므로 Office 공통 단축키라고 설명하지 않는다.
...
| 입력·부분 선택 | 커서/선택 영역, 범위에 맞는 도구 모음, `편집 마침`·`이번 편집 되돌리기` | 입력·부분 서식·버퍼 내 되돌리기. 마침/F2/Esc는 부모 로컬 초안 반영 승인 후 선택 상태 복귀. 명시적인 되돌리기만 해당 트랜잭션 취소 |
| 미완료 입력에서 바깥 클릭 | 기존 입력 유지, 마침/이번 편집 되돌리기 인라인 안내 | 도구 모음이면 선택 유지. 다른 블록 이동은 명시적인 마침 또는 이번 편집 되돌리기 후 수행 |
| 입력 마침 반영 실패 | 기존 버퍼·선택 유지, 실패 이유와 다시 마치기 | Esc/F2를 눌렀더라도 입력을 버리지 않는다. 같은 operation 재시도 또는 명시적인 이번 편집 되돌리기 |
...
DOM 보호 범위는 `compositionstart`–`compositionend`보다 길다. 부모의 edit grant로 입력을 시작한 때부터 commit/cancel의 결과와 재동기화 초안을 확인할 때까지 활성 입력 host와 그 소스 매핑을 고정한다. 입력 이벤트마다 React children·innerHTML을 다시 쓰지 않고, browser가 소유하는 plaintext 편집 DOM과 ref의 text/runs 버퍼를 사용한다. 조합 중 Enter·F2·Esc·Backspace를 편집 완료·취소·일반 단축키로 가로채지 않는다. Ctrl+Enter는 조합 여부와 관계없이 CMS 마침 명령으로 등록하지 않는다. `compositionend`는 한글 입력을 활성 버퍼 안에서 확정하는 시점이며 부모 로컬 초안에 편집을 반영하거나 서버에 저장하는 동작이 아니다. 최종 한글 한 음절의 조합을 여러 undo 항목으로 쪼개지 않는다.
...
활성 입력 동안 내용과 서식 변경을 같은 버퍼 history에 보관한다. `F2`, `편집 마침`, 또는 조합 중이 아닌 `Esc`는 모두 최종 text+runs replacement들을 `canvas-commit`, `outcome='apply'` 한 건으로 제출한다. 부모 CAS 승인과 재동기화를 확인한 뒤에만 확정된 로컬 초안 변경으로 취급하고 블록 선택 상태로 돌아간다. 실패·충돌·응답 유실 때는 활성 버퍼와 선택을 유지하며 Esc를 눌렀다는 이유로 폐기하지 않는다. 취소 프로토콜인 `outcome='cancel'`은 명시적인 `이번 편집 되돌리기` 버튼에만 연결한다. 취소는 부모 문서를 쓰지 않으며 부모 상태가 이미 바뀌어 CAS가 오래됐어도 가능해야 한다. 입력 진입 snapshot으로 돌아가는 것은 iframe 버퍼뿐이고, 재동기화 시에는 부모의 최신 초안을 표시한다. 이전 확정 트랜잭션은 건드리지 않는다. 일반적인 실수 복구는 Ctrl/Cmd+Z로 제공한다. 단순 포인터 선택이나 호버는 undo 항목으로 기록하지 않는다. 확정 트랜잭션에는 변경 전후 문서와 선택 위치를 보관하며 동일 필드의 copy와 runs를 따로 복원하지 않는다. 부모의 명령 반영이 iframe에 다시 도착해 동일 변경이 두 번 기록되는 것도 방지한다.
...
- [ ] `기존 확정 변경 → 새 입력 진입 → 입력·서식 변경 → Esc`에서 새 트랜잭션이 안전하게 부모 로컬 초안에 한 번 반영되고 이전 확정 변경도 유지되는 테스트를 작성한다. 승인·재동기화 전에는 입력 버퍼를 제거하지 않는다.
- [ ] `기존 확정 변경 → 새 입력 진입 → 입력·서식 변경 → 이번 편집 되돌리기`에서 현재 입력만 취소되고 부모 문서와 이전 확정 변경은 유지되는 테스트를 작성한다. 이후 명령 취소와 일반 undo를 혼동하지 않는지 확인한다.
- [ ] F2/Esc/편집 마침이 같은 apply 경로를 사용하며 저장·공개 RPC를 호출하지 않는지 확인한다. Ctrl+Enter는 마침을 호출하지 않아야 한다. 도구 모음·다른 블록·빈 캔버스 클릭은 활성 입력을 조용히 버리거나 확정하지 않아야 한다.
- [ ] 조합 중 Esc/F2/Enter가 apply/cancel을 발생시키지 않고 브라우저 조합 처리를 유지하는지 확인한다. 조합 종료 직후의 마침과 중복 키 입력도 한 번의 commit만 생성해야 한다.
...
| 직접 편집 | 블록 선택 → 부분 글자 선택 → 글꼴·크기 → 편집 마침 → undo → 입력 → Esc/F2 → 저장 → 새로고침 | 선택 위치·내용·서식·저장 범위 일치. Esc/F2는 안전한 로컬 반영 후 선택 복귀, 명시적인 이번 편집 되돌리기만 현재 입력 취소 |
| 원문 매핑 | 반복 단어, 행 trim, CRLF, 두 필드 결합, 이모지, 결합 글자 | 선택하지 않은 원문과 숨겨진 원문이 변하지 않음 |
| 키보드·터치 | Enter/F2/Esc/Tab, 편집 마침·이번 편집 되돌리기, 기본 선택 핸들, 가상 키보드 | 마침/명시적 취소 계약 일치, 반영 실패·바깥 클릭 시 입력 보존, 조합 중 단축키 가로채기 없음, Ctrl+Enter 마침 미등록, 터치 타깃 44px 이상 |
...
| 6. 키보드·접근성 | 마우스 없이 선택·입력·서식·확정·취소가 가능한가? | 포커스 표시, Enter 진입·F2/Esc 안전한 편집 마침·이번 편집 되돌리기 버튼 안내 |
...
- [x] 2026-09-18 문서 편집기 비교 조사에 따라 Esc/F2/편집 마침은 안전한 부모 로컬 초안 반영, 명시적인 이번 편집 되돌리기만 현재 입력 취소로 설명·프로토콜 매핑·예정 테스트를 동기화했다. Ctrl+Enter 마침은 제거했다. 실제 구현 검증 통과를 뜻하지 않는다.
- [x] 현재 구현 범위의 테스트·브라우저 결과와 실기기 미검증 범위를 별도 20회 검증 기록에 남겼다. 전체 직접 편집 coverage 완료 표시는 아니다.

### 2026-09-18 구현 상태

- 실제 preview의 exact/slice 원문 등록, 부모 grant/CAS, 일반 텍스트 입력, 부분 서식 6속성, 플로팅 도구, 활성 buffer 및 마친 편집 undo/redo를 연결했다.
- 색·굵기·기울임·장식을 기존 schemaVersion 1에 선택적으로 추가했다. 운영 DB에서는 validator만 확장했으며 공개/접수 데이터와 권한은 유지했다.
- 기존 캔버스의 동적 CMS 소유권과 복합 normalized/multipart 직접 입력은 아직 일반 exact/slice와 동일하게 처리하지 않는다. 기존 문구 목록·전용 관리의 fallback을 유지하며 Task 8 전체 완료를 주장하지 않는다.
- 기본 홈페이지 선택 시 줄 폭/배치 보존, 저장 중 입력 잠금, ACK 재동기화, 중복 operation, IME 모델 경계, 실제 Chromium 서식/저장/게시 흐름을 검증했다. 실제 OS IME·가상 키보드·Safari는 남은 검증이다.
- 영어 desktop 12개/mobile 2개 및 스크롤바 2안은 `docs/figma-english-scrollbar-approval-2026-09-18.md`에 연결했다. 실제 언어 전환·스크롤바 코드 적용은 승인 대기다.
