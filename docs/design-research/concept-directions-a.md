...
## 최종 스코프 결정 — 이 절이 아래 초기 탐색보다 우선한다

- **현행 full-bleed Hero와 핵심 슬라이드 애니메이션은 5안 모두 같은 구성으로 고정·보존한다.**
- 5안의 차이는 Hero를 교체하는 데 있지 않고 **첫 스크롤 아래**의 정보 구조에 있다: A 길찾기, B 프로그램 북, C 상태 우선 입단, D 필터 아카이브, E 참여·후원.
- 아래 초기 탐색에 남아 있는 `Cinematic Hero`, `Editorial Masthead`, `Question Hero`, `Artifact Hero`, `Manifesto Hero` 표현은 최종안에서 모두 **Hero 바로 아래 첫 모듈의 아트 디렉션**으로 읽는다. 현행 Hero를 없애거나 다른 Hero로 대체하라는 지시가 아니다.
- Hero에는 pause·이전·다음·현재 위치·`prefers-reduced-motion` 대응을 유지한다. 수정은 읽기·제어·성능을 강화하는 범위로 제한한다.
...
| 안 | 첫 스크롤 아래의 방향 | 가장 중요한 사용자 | Hero 아래 첫 과업 | 콘텐츠 밀도 | 대표 모션 | 핵심 CTA |
|---|---|---|---|---|---|---|
| A. 길찾기 우선 | 공연·입단·후원으로 빠르게 나뉘는 안내 | 공연 관람객·처음 온 방문자 | 다음 공연과 세 가지 주요 경로를 즉시 비교 | 낮음 | 현행 Hero crossfade + 선택 상태 전환 | `다가오는 무대 보기` |
| B. 프로그램 북 | 밝고 지적인 문화 에디토리얼 | 합창단 철학·역사를 알고 싶은 보호자·후원자 | 정체성과 신뢰를 읽게 함 | 중간 | 현행 Hero crossfade + line-draw·페이지 전환 | `합창단 이야기를 펼쳐보기` |
| C. 상태 우선 입단 | 모집 상태·대상·일정·절차 중심 | 입단 희망 학생·보호자 | `모집 중/마감/예정`과 다음 행동을 오해 없이 판단 | 중간~높음 | 현행 Hero crossfade + 탭 indicator·단계선 | `입단 대상과 절차 확인` |
| D. 필터 아카이브 | 절제된 미술관·기록관 | 기존 관객·동문·연구형 방문자 | 2014년 이후 청소년 기록을 연도·유형으로 탐색 | 중간 | 현행 Hero crossfade + filter·shared transition | `2014년부터 기록 탐색` |
| E. 참여·후원 | 따뜻한 공동체·공익 서사 | 후원자·지역사회·학교 파트너 | 사용처·문의·참여 방법을 근거와 함께 확인 | 중간 | 현행 Hero crossfade + 상태 기반 pulse·letter transition | `함께하는 방법 보기` |

---

## A. 길찾기 우선 — Current Hero + Wayfinding

> 최종 Figma 범위: 현행 full-bleed Hero를 그대로 두고, 아래에 적힌 무대감·공연 정보 아이디어는 Hero 직후의 길찾기 모듈에만 적용한다.
...
1. **공통 현행 100svh Hero 보존**
   - 현재 이미지·구성·crossfade·pause/prev/next/dots를 다섯 안과 동일하게 유지한다.
   - “NEXT STAGE”의 큰 날짜, 공연명, 장소, 상태, `공연 상세`는 Hero를 덮지 않고 첫 스크롤 아래 독립 길찾기 모듈로 제공한다.
2. **Three Ways**
...
- 현행 Hero와 intro 동작은 다른 네 안과 동일하게 보존하고, 첫 슬라이드 제목·설명·CTA·제어가 100svh 안에서 잘리지 않는지만 보강한다.
...
## B. 프로그램 북 — Living Program Book

> 최종 Figma 범위: 현행 full-bleed Hero를 그대로 두고, 프로그램 북 masthead는 Hero 직후 첫 스크롤 모듈로 배치한다.
...
1. **Hero 아래 Editorial Masthead**
   - 상단은 작은 `SEOUL MOTET YOUTH CHOIR / SINCE 2014`.
...
- 공통 현행 Hero 아래에서 masthead를 콘텐츠 높이에 맞게 구성해 목차가 과도하게 밀리지 않게 한다.
...
- `HomeHeroSlideshow`와 intro 동작은 다섯 안 공통으로 보존한다. B의 수동 page transition은 Hero 아래 프로그램 북에만 적용한다.
...
## C. 상태 우선 입단 — State-first Join

> 최종 Figma 범위: 현행 full-bleed Hero를 그대로 두고, Hero 직후에 모집 상태·지원 대상·일정·다음 행동을 먼저 보여 준다.
...
1. **Hero 아래 State-first Join**
   - 첫 줄에서 `모집 중/마감/예정`, 지원 대상, 접수·오디션 일시, 다음 행동을 텍스트로 표시한다.
   - 이어서 “합창을 처음 시작해도 괜찮을까요?” 같은 실제 질문과 학생·보호자 경로를 제공한다.
   - 개인 얼굴 대신 악보를 함께 보는 손, 원형 대형의 뒷모습, 파트 표식, 빈 연습 의자를 사용한다.
2. **Audience Switch**
...
7. **Application Letter**
   - 현재 `JoinCTA`를 페이지 전반의 결론으로 사용한다. CTA 옆에는 현재 확인된 모집 기간·오디션 일시·지원 대상·공식 문의처처럼 팩트북으로 검증된 정보만 둔다. 소요 시간, 제출 후 연락 방식, 수정 가능 여부는 실제 폼 동작과 운영 정책을 확인하기 전에는 표시하지 않는다.

### 모바일 390 재구성

- 공통 현행 Hero 아래에 상태 라벨 → 대상·일정 → 학생/보호자 2버튼 → 질문·설명 순으로 배치한다.
...
- `KineticHeadline`은 Hero 아래 질문의 두 번째 행에만 짧게 사용한다.
- `HomeHeroSlideshow`와 현행 제어는 다섯 안 공통으로 보존한다. 모집 상태 모듈은 자동 전환하지 않는다.
- `JoinCTA`는 현재 outline·버튼·요약 dl 구조를 유지하되 카드 그림자를 줄인다.
- Scorebook·archive flutter는 상태 우선 입단 모듈에 추가하지 않는다. Hero intro는 공통 현행 동작을 따른다.
...
## D. 필터 아카이브 — Filtered Archive

> 최종 Figma 범위: 현행 full-bleed Hero를 그대로 두고, Hero 직후에 연도·자료 유형 필터와 대표 기록을 배치한다.
...
기존 관객, 동문, 보호자, 문화 관계자가 청소년합창단의 2014년 이후 활동을 연도·공연·매체로 탐색하고, 성인 서울모테트합창단이 1989년부터 쌓아 온 음악적 유산과 청소년 교육의 관계를 서로 다른 시간축으로 이해한다.
...
1. **Hero 아래 Featured Artifact**
   - 왼쪽 5열에 `2014—2025`, 제목, 한 문장, CTA. 2025는 공개 자료로 확인된 제11회 정기연주회까지의 공연 아카이브 기준이며, 이후 검증된 기록이 추가되면 데이터에서 갱신한다.
...
- Hero 아래 대표 자료는 `contain` 3:4 또는 실제 비율로 보여주고 배경 cover로 잘라 쓰지 않는다.
...
- `HomeHeroSlideshow`와 intro 동작은 다섯 안 공통으로 보존한다. Hero 아래 아카이브에는 별도 자동 슬라이드·scorebook flutter를 추가하지 않는다.
...
- Primary: **2014년부터 기록 탐색** → `/gallery`
- Secondary: **다가오는 공연 보기** → `/concerts`
- Community: **기록을 함께 잇기** → `/contact`

---

## E. 참여·후원 — Participation & Support

> 최종 Figma 범위: 현행 full-bleed Hero를 그대로 두고, Hero 직후에 참여 경로·후원 사용처·문의·청소년 보호 원칙을 근거와 함께 배치한다.
...
1. **Hero 아래 Participation Manifesto**
...
- `KineticHeadline`: Hero 아래 Manifesto 두 행 중 한 행만 수평 진입.
...
- `HomeHeroSlideshow`와 intro 동작은 다섯 안 공통으로 보존한다. 참여·후원 모듈에는 별도 scorebook flutter를 추가하지 않는다.
...
각 콘셉트마다 같은 규격의 Desktop, Mobile, Reduced motion, Notes 프레임을 만든다.

### Desktop / 1440×2100

- Frame: `1440 × 2100`, 공통 현행 Hero와 첫 스크롤 아래 핵심 과업 구조를 같은 높이에서 비교
...
- Frame: `390 × 1688`, Mobile과 동일 콘텐츠·순서
- Hero·아래 모듈의 이동·scale을 제거하고 즉시 전환 또는 짧은 opacity로 대체
- 현재 위치·선택·진행·오류·마감 상태는 Full motion과 동일하게 보존

### Notes / 520×1100

- Frame: `520 × 1100`
- 사이트·리소스 근거 ID, 가져온 장점, 피한 단점, 모션 목적, 미확정 사실을 기록
...
한 안을 전체 홈페이지 스킨으로 채택하지 않는다. **현행 full-bleed Hero를 공통 고정**하고, 첫 스크롤 아래는 현재 가장 중요한 운영 과업에 따라 A 또는 과업별 모듈을 조합한다.

- A에서: 다음 공연과 `공연 / 입단 / 후원·문의` 길찾기
- B에서: 흰 프로그램 북 구조, scorebook, 역사·정신의 읽기 경험
- C에서: 모집 상태, 학생/보호자 경로, 입단 단계
- D에서: 포스터 비율을 보존하는 archive와 연도·유형 필터
- E에서: 가치→실제 활동→후원 사용처·문의 연결

이 조합은 현재 Hero의 무대감과 애니메이션을 버리지 않으면서, 아래 정보면에서 흰 바탕·실제 오렌지 심볼·명확한 과업 순서를 적극 사용한다. 카드·그림자·장식의 양은 줄이고 기존 애니메이션은 방향·상태·진행을 설명하도록 다듬는다.
